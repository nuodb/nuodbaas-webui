...
function SqlImportTab({
  ...
  function renderFileSelector() {
    return (
      <div className="NuoColumn NuoCenter">
        <input
          className="NuoUpload"
          id="file-upload"
          type="file"
                ...
                if (
                  !updatedFiles.find(
                    (f) =>
                      f.name === flItem.name &&
                      f.size === flItem.size &&
                      f.lastModified === flItem.lastModified,
                  )
                ) {
    ...
    return new Promise((resolve, reject) => {
      let headers = {
        Authorization:
          "Basic " +
          btoa(
            sqlConnection.getDbUsername() + ":" + sqlConnection.getDbPassword(),
          ),
      };
      Rest.getStream(
              ...
              if (stats.progressKey && !gotProgressKey) {
                ...
                if (task) {
                  ...
                  if (task.data && task.data.file && stats.bytesProcessed) {
                    task.percent =
                      (stats.bytesProcessed * 100) / task.data.file.size;
                  }
        ...
        .catch((error) => {
          if (error.status) {
            Toast.show(
              "Cannot get progress monitoring",
  ...
  async function addToQueue(toQueue: File[]) {
    let newTasks: BackgroundTaskType[] = toQueue.map((file) => {
      return {
        ...
        execute: async (task: BackgroundTaskType) => {
          ...
          const stats = await sqlConnection.sqlImport(
            task.data.file,
            progressKey,
            progressAbortController,
          );
        ...
        showMinimal: (
          ...
          return (
            <div key={task.data.file.name} className="NuoRow">
    ...
    for (let i = 0; i < toQueue.length; i++) {
      newFiles = newFiles.filter(
        (f) =>
          f.name !== toQueue[i].name ||
          f.lastModified != toQueue[i].lastModified ||
          f.size != toQueue[i].size,
      );
    }
            ...
            return (
              <TableRow key={file.name}>
                ...
                <TableCell className="NuoColumn">
                  <div className="NuoUploadLightLabel">
                    {shortenSize(file.size)}
                  </div>
                  <div className="NuoUploadLightLabel">
          ...
          {files.length === 0 ? null : (
            <TableRow>
              ...
              <td>
                ...
                >
                  {t("button.sql.addAllToQueue")}
                </Button>
              </td>
            </TableRow>
          )}
          {tasks
            .filter((t) => t.id.startsWith(TASK_ID_PREFIX))
            .map((task) => {
              return (
                <TableRow key={task.data.file.name}>
                  <TableCell>{task.data.file.name}</TableCell>
                  <TableCell className="NuoColumn">
                    <div className="NuoUploadLightLabel">
                      {shortenSize(task.data.file.size)}
                    </div>
                    <div className="NuoUploadLightLabel">
                      {new Date(
                        task.data.file.lastModified,
                      ).toLocaleDateString()}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    {task.data.error ||
                      (task.data.failedQueries &&
                        task.data.failedQueries.length && (
                          <details>
                            <summary>Failed Queries:</summary>
                            {task.data.failedQueries.map(
                              (fq: string, index: number) => (
                                <div key={index}>{fq}</div>
                              ),
                            )}
                          </details>
                        )) ||
