...
export default function SqlRegisterUser({
  onClose,
  organization,
  project,
  database,
}: SqlRegisterUserProps) {
  const [adminUsername, setAdminUsername] = useState("");
  const [adminPassword, setAdminPassword] = useState("");
  const [conn, setConn] = useState<SqlType | undefined>(undefined);
  const [error, setError] = useState<string | undefined>();
  const newDbUser = Auth.getCredentials()
    ?.username.replace("/", "_")
  ...
  useEffect(() => {
    ...
    conn.runCommand("EXECUTE_QUERY", [sql]).then((sqlResponse) => {
      if (
        ...
        Toast.show(
          "Cannot retrieve roles: " + sqlResponse.error,
          sqlResponse.error,
        );
        setRoles({});
      }
  ...
  return (
    <DialogMaterial open={true}>
      <DialogTitle>
        {t("form.sqleditor.label.setupLogin", {
          database: organization + "/" + project + "/" + database,
        })}
      </DialogTitle>
      <DialogContent>
        <div>
      ...
      <DialogActions>
        ...
        {conn && (
          <Button
            data-testid="dialog_button_register"
            onClick={async () => {
              let sql = "START TRANSACTION;";
              sql += "CREATE USER " + sqlIdentifier(newDbUser) + " EXTERNAL;";
              Object.keys(roles)
                .filter((role) => roles[role] !== "disabled")
                .forEach((role) => {
                  sql +=
                    "GRANT " +
                    sqlIdentifier(role) +
                    " TO " +
                    sqlIdentifier(newDbUser);
                  if (roles[role] === "grant") {
              ...
              sql += "COMMIT;";
              const response: SqlImportResponseType =
                await conn.sqlSimpleImport(sql);
