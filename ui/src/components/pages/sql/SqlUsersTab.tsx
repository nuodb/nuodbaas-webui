...
function SqlUsersTab({ sqlConnection, t }: SqlUsersTabProps) {
  ...
  const [editDialogProps, setEditDialogProps] =
    useState<EditDialogProps>(undefined);
  ...
  async function refreshResults(args: SqlResponseState) {
    ...
    if (orderBy) {
      sqlQuery +=
        " ORDER BY `" + orderBy + "` " + (isAscending ? "ASC" : "DESC");
    }
    const sqlQueryWithLimit =
      ...
      " offset " +
      String((page - 1) * DEFAULT_PAGE_SIZE);
    ...
    if (
      ...
      sqlResponse.rows &&
      sqlResponse.rows.length === DEFAULT_PAGE_SIZE
    ) {
      const countResults = await sqlConnection.runCommand("EXECUTE_QUERY", [
        "SELECT count(*) FROM (" + sqlQuery + ") total",
      ]);
      if (
        countResults.status === "SUCCESS" &&
        countResults.rows &&
        countResults.rows[0] &&
        countResults.rows[0].values
      ) {
  ...
  function renderError() {
    return (
      editDialogProps?.error && (
        <div className="NuoError">{editDialogProps.error}</div>
      )
    );
  }
  ...
  function renderCreateDialog() {
    ...
    return (
      <DialogMaterial open={true}>
        ...
        <DialogContent>{renderError()}</DialogContent>
        <DialogActions>
          <Button
            data-testid="dialog_button_local"
            onClick={() => {
              setEditDialogProps({ ...editDialogProps, type: "create_local" });
            }}
          >
            {t("form.sqleditor.button.createDatabaseUser")}
          </Button>
          <Button
            data-testid="dialog_button_dbaas"
            onClick={async () => {
              const users: string[] = (
                (await Rest.get("/users?listAccessible=true")) as any
              ).items;
              const existingUsers: string[] =
                state.sqlResponse?.rows?.map((row) =>
                  row.values[0].replace("_", "/"),
                ) || [];
              ...
              const loggedInOrg =
                Auth.getCredentials()?.username.split("/")[0] || "";
              ...
              setEditDialogProps({
                ...editDialogProps,
                type: "add_dbaas",
                allDbaasUsers: sortedUsers,
              });
            }}
          >
            {t("form.sqleditor.button.addDbaasUser")}
          </Button>
          <Button
      ...
      <DialogMaterial open={true}>
        ...
        <DialogContent>
          <div className="NuoColumn">
            ...
            <SqlRoleSelector
              roles={editDialogProps.roles}
              setRoles={(roles) =>
                setEditDialogProps({ ...editDialogProps, roles })
              }
            />
            {renderError()}
          </div>
        ...
        <DialogActions>
          <Button
            ...
            onClick={async () => {
              ...
              sql +=
                "CREATE USER " +
                sqlIdentifier(editDialogProps.username) +
                " PASSWORD " +
                sqlString(editDialogProps.password) +
                ";";
              ...
              if (sqlResponse.status === "SUCCESS") {
                Toast.show(
                  "User " + editDialogProps.username + " created",
                  undefined,
                );
                ...
                setEditDialogProps({
                  ...editDialogProps,
                  error: "User creation failed: " + sqlResponse.error,
                });
              }
            }}
          >
  ...
  function renderAddDbaasDialog() {
    ...
    return (
      <DialogMaterial open={true}>
        <DialogTitle>
          {t("form.sqleditor.label.addDbaasUserToDatabase")}
        </DialogTitle>
        <DialogContent>
          <div className="NuoColumn">
            <div className="NuoFieldContainer">
              ...
              >
                {editDialogProps.allDbaasUsers?.map((user) => (
                  <SelectOption value={user.replace("/", "_").toLowerCase()}>
                    {user.toLowerCase()}
                  </SelectOption>
                ))}
              </Select>
            </div>
            {editDialogProps.username && (
              <SqlRoleSelector
                roles={editDialogProps.roles}
                setRoles={(roles) =>
                  setEditDialogProps({ ...editDialogProps, roles })
            ...
            onClick={async () => {
              ...
              if (sql) {
                ...
                if (sqlResponse.status === "SUCCESS") {
                  ...
                  setEditDialogProps({
                    ...editDialogProps,
                    error: "Adding DBaaS user failed: " + sqlResponse.error,
                  });
                }
              }
            }}
  ...
  function renderEditDialog() {
    ...
    return (
      <DialogMaterial open={true}>
        <DialogTitle>
          {t("form.sqleditor.label.editUser", {
            username: editDialogProps.username,
          })}
        </DialogTitle>
        <DialogContent>
          <div className="NuoColumn">
        ...
        <DialogActions>
          <Button
            ...
            onClick={async () => {
              ...
              Object.keys(editDialogProps.roles).forEach((role) => {
                if (
                  editDialogProps.roles[role] ===
                  editDialogProps.origRoles[role]
                ) {
                  // no change
              ...
              if (sql) {
                ...
                if (sqlResponse.status === "SUCCESS") {
                  ...
                  setEditDialogProps({
                    ...editDialogProps,
                    error: "User update failed: " + sqlResponse.error,
                  });
                }
              }
            }}
  ...
  async function handleDelete(username: string) {
    ...
    if (
      ...
      (await Dialog.confirm(
        ...
        t("form.sqleditor.label.delete.user.body", i18nData),
        t,
      ))
    ) {
      sqlConnection
        .runCommand("EXECUTE", ["DROP USER " + sqlIdentifier(username)])
        .then((sqlResponse) => {
          if (sqlResponse.status === "SUCCESS") {
            Toast.show(
              t("form.sqleditor.label.user.deleted", i18nData),
              undefined,
            );
            refreshResults(state);
          } else {
            Dialog.ok(
              t("form.sqleditor.label.user.delete.failed", i18nData),
              sqlResponse.error,
              t,
            );
          }
      ...
      <SqlResultsRender
        ...
        onAdd={async () => {
          const roles = await getRoles(
            ...
            (error: string) =>
              editDialogProps &&
              setEditDialogProps({ ...editDialogProps, error }),
          );
          setEditDialogProps({
            roles,
            origRoles: roles,
        ...
        onDeleteDisabled={(username: string) =>
          equalsIgnoreCase(username, "dba") ||
          equalsIgnoreCase(username, sqlConnection.getDbUsername())
        }
        addLabel={
          <>
            <AddIcon />
