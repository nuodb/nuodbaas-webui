...
function SqlUsersTab({ sqlConnection, t }: SqlUsersTabProps) {
    ...
    async function refreshResults(args: SqlResponseState) {
        ...
        let sqlQuery = "select p.username,ur.rolename,substring(p.verifier,1,1), pr.objectname, pr.privilegemask, pr.* from system.passwords p left join system.userroles ur on p.username = ur.username left join system.privileges pr on ur.username = pr.holdername order by p.username,ur.rolename,pr.objectname";
