    ...
    <form>
        ...
        showRegisterUserDialog && (
          <SqlRegisterUser
            ...
            onClose={async (action: string) => {
              ...
              const authUsername = Auth.getCredentials()?.username.replace(
                "/",
                "_",
              );
              ...
              if (
                ...
                try {
                  const response: SqlResponse = await conn.runCommand(
                    "EXECUTE_QUERY",
                    ["SELECT 1 FROM DUAL"],
                  );
      ...
      <div className="NuoFieldContainer">
        <TextField
          ...
          ) => setDbUsername(event.currentTarget.value)}
        />
      </div>
      <div className="NuoFieldContainer">
        <TextField
      ...
      <div className="NuoFieldContainer NuoButtons">
        ...
        >
          {t("form.sqleditor.button.login")}
        </Button>
        {showRegistration && (
          <Button
            data-testid="sql.setupLogin.button"
            variant="outlined"
            onClick={async () => {
              setShowRegisterUserDialog(true);
            }}
          >
            {t("form.sqleditor.button.setupLogin")}
          </Button>
        )}
      </div>
