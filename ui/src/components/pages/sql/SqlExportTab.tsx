...
import {
  ...
  generateRandom,
  isTaskFinished,
  launchNextBackgroundTask,
  shortenSize,
  updateOrAddTask,
} from "../../../utils/BackgroundTasks";
import BackgroundTaskStatus, {
...
function SqlExportTab({ tasks, setTasks, sqlConnection }: SqlExportTabProps) {
  ...
  const [exportTables, setExportTables] = useState<string[] | undefined>(
    undefined,
  );
  ...
  const [foregroundTaskId, setForegroundTaskId] = useState<string | undefined>(
    undefined,
  );

  useEffect(() => {
    sqlConnection
      .runCommand("EXECUTE_QUERY", [
      ...
      <DialogMaterial open={true}>
        ...
        <DialogContent>
          <div className="NuoColumn" style={{ alignItems: "center" }}>
            <div>
              {shortenSize(task.data.downloaded)}{" "}
              {isTaskFinished(task) ? "downloaded" : "downloaded so far"}
            </div>
        ...
        <DialogActions>
          <Button
            data-testid="export.status.button"
            variant="text"
            onClick={() => {
              ...
              const newTasks = tasks.filter(
                (t: BackgroundTaskType) => t.id != task.id,
              );
              setTasks(newTasks);
            }}
          >
            {(isTaskFinished(task) && t("button.dismiss")) ||
              t("button.cancel")}
          </Button>
          {!isTaskFinished(task) && (
            <Button
  ...
  async function exportFile(sqlConnection: SqlType): Promise<string> {
    const showSaveFilePicker =
      localStorage.getItem("selenium") === "true"
        ? fakeShowSaveFilePicker
    ...
    const body = {
      ...
      outputTable:
        (exportTables && exportTables.map((et) => outputTables[et] || et)) ||
        undefined,
    ...
    let newTask: BackgroundTaskType = {
      ...
      status: "not_started",
      description: t("form.sqleditor.label.exporting", {
        dbSchema: sqlConnection.getOrgProjDbSchemaUrl(),
      }),
        ...
        const response = await fetch(url, {
          ...
          headers: {
            Authorization:
              "Basic " +
              btoa(
                sqlConnection.getDbUsername() +
                  ":" +
                  sqlConnection.getDbPassword(),
              ),
      ...
      showMinimal: (
        ...
        return (
          <div key={task.label} className="NuoRow">
            <div className="NuoRow">
              {task.label} {shortenSize(task.data.downloaded)}
            </div>
            <div className="NuoRowFixed">
              <BackgroundTaskStatus
                ...
                tasks={tasks}
                setTasks={setTasks}
                abortController={progressAbortController}
              />
            </div>
          </div>
        );
      ...
      <form>
        <div className="NuoColumn">
          ...
          <div className="NuoFieldContainer">
            <TextField
              id="outputSchema"
              label={t("form.sqleditor.label.renameSchema")}
              value={outputSchema || ""}
              onChange={(event) => {
          ...
          <div className="NuoFieldContainer">
            <TextField
              id="batchSize"
              label={t("form.sqleditor.label.batchSize")}
          ...
          <div className="NuoFieldContainer">
            <TextField
              id="exportLimit"
              label={t("form.sqleditor.label.exportLimit")}
