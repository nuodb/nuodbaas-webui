  ...
  if (username) {
    sql =
      `
        ...
        FROM system.roles r
        LEFT JOIN (
            SELECT username,rolename,options FROM system.userroles WHERE username=` +
      sqlString(username) +
      `
        ) ur
  ...
  if (
    ...
    sqlResponse.rows.forEach((row) => {
      if (row.values.length === 4) {
        ...
        if (username) {
          const enabled =
            String(row.values[2]).toLowerCase() === username.toLowerCase();
          roles[role] = enabled
            ? row.values[3] === 1
              ? "grant"
              : "enabled"
...
export default function SqlRoleSelector({
  roles,
  setRoles,
}: RoleSelectorProps) {
  return (
    <table className="NuoSimpleTable">
      <thead>
        <tr>
          <th>User Roles</th>
          <th>Grant Option</th>
      ...
      <tbody>
        {Object.keys(roles).map((roleKey) => (
          <tr key={roleKey}>
            <td>
              <input
                data-testid={"user-roles-" + roleKey}
                type="checkbox"
                name={roleKey}
            ...
            <td>
              <input
                data-testid={"grant-option-" + roleKey}
                type="checkbox"
                name={roleKey}
