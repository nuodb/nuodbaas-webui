...
function SqlResultsRender({
  ...
  function renderMenuCell(value: string, zIndex: number, t: any) {
    ...
    return (
      ...
      >
        <Menu
          data-testid="resource-popup-menu"
          popupId={"row_menu_" + value}
          items={buttons}
          align="right"
        />
      </TableCell>
  ...
  return (
    <Table data-testid="table_sql">
      <TableHead>
        <TableRow>
          {results.columns?.map((column, index: number) => (
            <TableTh
              key={index}
              data-testid={column.name}
              className={
                (onEdit || onDelete) && index === 0 ? "NuoStickyLeft" : ""
              }
            >
          ...
          {isFiltered !== undefined && (
            <TableTh className="NuoTableMenuCell NuoStickyRight">
              <div
                className={isFiltered ? "NuoFilterActive" : "NuoFilterInactive"}
                onClick={() => setShowFilterDialog && setShowFilterDialog(true)}
              >
                <FilterListIcon />
              </div>
            </TableTh>
          ...
          {onAdd && addLabel && (
            <TableTh
              ...
              className="NuoTableMenuCell NuoStickyRight"
            >
              <div className="NuoColumn NuoRight">
                <button data-testid="sql-add-button" onClick={() => onAdd()}>
                  {addLabel}
                </button>
              </div>
            </TableTh>
      ...
      <TableBody>
        {results.rows?.map((row, index) => (
          <TableRow key={index}>
            {row.values.map((col, cindex) => (
              <TableCell
                className={onEdit && cindex === 0 ? "NuoStickyLeft" : ""}
                key={cindex}
              >
                {onEdit && cindex === 0 ? (
                  <button
                  ...
                  <div style={{ whiteSpace: "pre" }}>{String(col).trim()}</div>
                )}
              </TableCell>
            ))}
            {renderMenuCell(
              row.values[0],
              1000 + (results.rows || []).length - 1 - index,
              t,
            )}
          </TableRow>
        ))}
