...
function SqlBrowseTab({ sqlConnection, table, t }: SqlBrowseTabProps) {
    ...
    if (sqlResponse.columns && filter !== undefined) {
      ...
      sqlResponse.columns.forEach((column) => {
        if (column.name in filter) {
          ...
          initFilter[column.name] = {
            type: getFilterConditionOptions(column.type)[0].id,
            value: "",
          };
        }
      });
  ...
  const isFiltered =
    ...
    !!Object.keys(state.filter).find(
      (key) => !!(state.filter && state.filter[key].value),
    );
  return (
    <div className="NuoTableScrollWrapper">
      {showFilterDialog && state.filter && (
        <SqlFilter
          columns={state?.sqlResponse?.columns || []}
          filter={state.filter}
          setFilter={(newFilter) => {
            if (newFilter) {
              const newState = {
