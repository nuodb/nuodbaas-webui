...
function Automation(props: PageProps) {
  ...
  const filteredLog = log.filter((entry) =>
    hideGetRequests ? entry.method !== "get" : true,
  );
  const token = Auth.getCredentials()?.token;

  let selectedLogEntry = filteredLog.find(
    (fl) => fl.timestamp === selectedTimestamp,
  );
    ...
    <PageLayout {...props}>
      <div className="NuoContainerLG">
        ...
        <div className="NuoButtons">
          ...
          >
            {t("dialog.automation.startRecording")}
          </Button>
          <Button
            data-testid="btnStopRecording"
            disabled={!isRecording}
            variant="contained"
            onClick={() => {
          ...
          >
            {t("dialog.automation.stopRecording")}
          </Button>
        </div>
          ...
          <div className="NuoColumn">
            ...
            {filteredLog.length > 0 ? (
              <div className="NuoRow">
                ...
                {selectedLogEntry && (
                  <textarea
                    value={JSON.stringify(selectedLogEntry, null, 2)}
                    readOnly={true}
                    disabled={true}
                    rows={20}
                    cols={80}
              ...
              t("dialog.automation.noWriteRequests")
            )}
          </div>
        )}
        ...
        {filteredLog.length > 0 &&
          !isRecording &&
          renderCopyCode(
            t("dialog.automation.curl"),
            getCurlCommands(filteredLog),
          )}
      </div>
    </PageLayout>
  );
  ...
  function getCurlCommands(log: RestLogEntry[]): string[] {
    ...
    log.forEach((entry) => {
      ...
      let curl =
        "curl -X " +
        method +
        ' -H "Authorization: Bearer $AUTH_TOKEN" -H "Content-Type: ' +
        contentType +
        '" "$BASE_URL' +
        url +
        '"';
