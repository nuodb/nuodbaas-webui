...
export default function ChangeDbaPassword({
  ...
  async function handleChangePasswordButton(): Promise<boolean> {
    ...
    const dbName = fullPath.startsWith("/databases/")
      ? fullPath.substring("/databases/".length)
      : fullPath;
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        await Rest.post(fullPath + "/dbaPassword", {
          current: passwords["oldPassword"],
          target: passwords["newPassword1"],
        });
        Toast.show(
          t("dialog.databasePassword.passwordChanged", { database: dbName }),
          undefined,
        );
        return true;
      } catch (error) {
        if (axios.isAxiosError(error)) {
          setDialogError(
            t("dialog.databasePassword.cannotChangePassword", {
