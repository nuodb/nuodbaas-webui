...
export default function DbConnectionInfo({ data, t }: DbConnectionInfoProps) {
  ...
  const jdbcJava = [
    ...
    '    final static String JDBC_URL = "jdbc:com.nuodb://' +
      sqlEndpoint +
      ":" +
      port +
      "/" +
      dbName +
      '?trustStorePassword=" + JKS_PASSWORD_URLENCODED + "&trustStore=" + JKS_PATH_URLENCODED;',
    "",
  ...
  const jdbcSh = [
    ...
    'keytool -import -trustcacerts -alias "' +
      ...
      '" -file "' +
      dbName +
      '.pem" -keystore nuodb.jks -storepass passw0rd -noprompt',
  ...
  function renderCopyCode(
    ...
    return (
      <Accordion data-testid={dataTestid} summary={summary}>
        <Tooltip
          title={
        ...
        <textarea
          ...
          disabled={true}
          className="NuoDbConnectionInfoSample"
          value={lines.join("\n")}
        ></textarea>
      </Accordion>
    );
  ...
  function processMarkdownLinks(text: string): React.ReactNode {
    ...
    while (text) {
      ...
      ret.push(
        <a
          key={ret.length}
          href={text.substring(posBetween + "](".length, posLink)}
        >
          {text.substring(posTitle + "[".length, posBetween)}
        </a>,
      );
  ...
  return (
    <div className="NuoDbConnectionInfo">
      ...
      <Accordion
        data-testid="codesamples"
        summary={t("dialog.dbConnectionInfo.label.codeSamples")}
        defaultExpanded={false}
      >
        <div>
          {processMarkdownLinks(t("dialog.dbConnectionInfo.label.description"))}
        </div>
        {renderCopyCode(
          "nuosql",
          t("dialog.dbConnectionInfo.label.nuosql"),
          nuosql,
        )}
        <Accordion
          data-testid="jdbc"
          summary={t("dialog.dbConnectionInfo.label.jdbc")}
          defaultExpanded={false}
        >
        ...
        <Accordion
          data-tesetid="cpp"
          summary={t("dialog.dbConnectionInfo.label.cpp")}
          defaultExpanded={false}
        >
