...
function Organization({
  ...
  return (
    <ComboBox
      loadItems={() => {
        return new Promise((resolve) => resolve(orgMenuItems));
      }}
      selected={org}
    >
      <CorporateFareIcon />
      <label>{org === "" ? t("field.select.allOrgs") : org}</label>
    </ComboBox>
  );
}
        ...
        <div className="details" key={key}>
          ...
          <ol>
            {Object.keys(data[key].children).map((childKey) => {
              ...
              return (
                <li
                  data-testid={"menu-button-" + childKey}
                  key={childKey}
                  className={className}
                  tabIndex={0}
                  onClick={() => {
                ...
                >
                  {icon}
                  <label>{data[key].children[childKey].label}</label>
                </li>
              );
            })}
          </ol>
  ...
  resources.forEach((resource) => {
    if (resource.startsWith("cluster/")) {
      childrenCluster[resource] = {
        ...
        label: t(
          "resource.label." + resource.replace("/", "."),
          resource.substring("cluster/".length),
        ),
