...
export class Rest extends React.Component<{
  ...
  static decrementPending() {
    ...
    instance.setState((prevState: State) => {
      return {
        pendingRequests:
          prevState.pendingRequests > 0 ? prevState.pendingRequests - 1 : 0,
      };
    });
  }
  ...
  static log(
    ...
    automationLog.push({
      timestamp: instance.lastTimestamp.toISOString(),
      method,
      url,
      body,
      success,
  ...
  render(): ReactNode {
    return this.state.pendingRequests > 0 ? (
      ...
      <div
        data-testid="rest_spinner__complete"
        className="RestSpinner"
        style={{ zIndex: -1 }}
      >
        &nbsp;
      </div>
  ...
  static async get(path: string) {
    return new Promise((resolve, reject) => {
      ...
      axios
        .get(url, { headers: Auth.getHeaders() })
        .then((response) => {
  ...
  static async put(path: string, data: JsonType) {
    return new Promise((resolve, reject) => {
      ...
      const url = Auth.getNuodbCpRestUrl(path);
      axios
        .put(url, data, { headers: Auth.getHeaders() })
        .then((response) => {
  ...
  static async post(path: string, data: JsonType) {
    return new Promise((resolve, reject) => {
      ...
      const url = Auth.getNuodbCpRestUrl(path);
      axios
        .post(url, data, { headers: Auth.getHeaders() })
        .then((response) => {
  ...
  static async delete(path: string) {
    return new Promise((resolve, reject) => {
      ...
      const url = Auth.getNuodbCpRestUrl(path);
      axios
        .delete(url, { headers: Auth.getHeaders() })
        .then((response) => {
