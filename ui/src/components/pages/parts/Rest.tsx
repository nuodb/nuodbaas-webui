...
export class Rest extends React.Component<{ isRecording: boolean, setIsRecording: (isRecording: boolean) => void }> {
    ...
    static async isCorsError(url: string, reason: any) {
        if (url.startsWith("/") || url.toLowerCase().startsWith((window.location.protocol + window.location.hostname + "/").toLowerCase())) {
            return false;
        }
            ...
            axios.get(url, { headers: Auth.getHeaders() })
                .then(response => {
                    ...
                    if (await Rest.isCorsError(url, reason)) {
                        const button = await Dialog.show("Network error", "Unable to load resources from other domain. What do you want to do?", [{
                            id: "redirect",
                            label: "Redirect to " + url.split("/")[2] /* 2 = hostname portion of the path */
                        },
                        {
                            id: "reset",
                        ...
                        if (button === "redirect") {
                            ...
                            window.location.href = url.split("/").slice(0, 3).join("/") + "/u" + (new Date().getTime() > 0 ? "" : "never_occurs") + "i/"
                        }
                        else {
