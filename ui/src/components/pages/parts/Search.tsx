      ...
      if (type === "object") {
        if (definition[key].additionalProperties) {
          ...
          if (
            ...
            console.log(
              "INVALID additionalProperties type",
              definition[key],
              key,
              type,
            );
        ...
        if (definition[key].items?.type === "string") {
          ...
          if (definition[key].items.additionalProperties) {
            ...
            if (
              ...
              console.log(
                "INVALID INNER additionalProperties type",
                definition[key],
                key,
                type,
              );
...
function Search({ path, search, setSearch, fieldNames, t }: SearchProps) {
  ...
  return (
    ...
    >
      <ListResourceFilter
        editIndexOrNewField={editIndexOrNewField}
        fields={fields}
        search={search}
        setSearch={(search: SearchType[] | null) => {
      ...
      <CreatableSelect
        ...
        onChange={(newValue: any, actionMeta: ActionMeta<SearchType>) => {
          if (actionMeta.action === "select-option" && actionMeta.option) {
            ...
            setSearch(
              search.filter((s) => !isSameSearch(s, actionMeta.removedValue)),
            );
          } else {
            setSearch(newValue);
          }
        ...
        formatOptionLabel={(
          data: SearchType,
          meta: FormatOptionLabelMeta<SearchType>,
        ) => {
