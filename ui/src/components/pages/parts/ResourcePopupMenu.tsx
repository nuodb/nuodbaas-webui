...
export default function ResourcePopupMenu({
  ...
  async function handleDelete(row: TempAny, deletePath: string) {
    ...
    row = {
      ...row,
      resources_one: t(
        "resource.label." + createPathFirstPart + "_one",
        createPathFirstPart,
      ),
    };
    if (
      ...
      (await Dialog.confirm(
        t("confirm.delete.resource.title", row),
        t("confirm.delete.resource.body", row),
        t,
      ))
    ) {
  ...
  if (
    ...
    "get" in resource &&
    row["$ref"] &&
    Auth.hasAccess("GET", editDeletePath, sla)
  ) {
    buttons.push({
  ...
  if (
    resource &&
    "put" in resource &&
    Auth.hasAccess("PUT", editDeletePath, sla)
  ) {
    buttons.push({
  ...
  if (
    resource &&
    "delete" in resource &&
    Auth.hasAccess("DELETE", editDeletePath, sla)
  ) {
    buttons.push({
  ...
  if (cv && cv.menu) {
    cv.menu.forEach((menu: CustomViewMenu) => {
      ...
      if (
        menuVisible &&
        menu.patch &&
        !Auth.hasAccess("PATCH", editDeletePath, sla)
      ) {
        menuVisible = false;
      }
      if (
        menuVisible &&
        menu.writeAccessRequired &&
        !Auth.hasAccess("PUT", editDeletePath, sla)
      ) {
        menuVisible = false;
      }
          ...
          onClick: () => {
            ...
            promiseConfirm.then((result) => {
              if (result === "yes") {
                if (menu.patch) {
                  Rest.patch(
                    row["$ref"] ? path + "/" + row["$ref"] : path,
                    menu.patch,
                  ).catch((error) => {
                    Toast.show(
                      "Unable to update " + path + "/" + row["$ref"],
                      error,
                    );
                  });
                } else if (menu.link) {
