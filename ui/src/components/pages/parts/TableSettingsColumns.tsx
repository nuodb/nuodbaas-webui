...
async function saveColumns(cols: MenuItemProps[], path: string) {
  ...
  mergeCustomizations({
    views: {
      [schemaPath]: {
        columns: cols
          .filter((col) => col.id !== "$ref" && col.selected)
          .map((col) => col.id),
      },
    },
  });
...
function TableSettingsColumns(props: TempAny) {
  ...
  const items = columns
    .filter((column: MenuItemProps) => column.id !== "$ref")
    .map((column: MenuItemProps, index: number) => {
      return {
        id: column.id,
        ...
        label: (
          ...
          >
            ...
            <label htmlFor={column.id}>
              {t("field.label." + column.id, column.id)}
            </label>
          </div>
        ),
      };
