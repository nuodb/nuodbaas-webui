...
function ResourceHeader({ schema, path, data, type, filterValues, onAction, t }: ResourceHeaderProps) {
    ...
    const createPathFirstPart = createPath?.split("/")[1].replace(/^cluster\//, "cluster.");
    ...
    return <div className="NuoListResourceHeader">
        ...
        <div>
            ...
            <div className="NuoRow" style={{ justifyContent: "end" }}>
                ...
                {type === "list" && createPath && Auth.hasAccess("PUT", createPath, undefined) && <div className="Nuo-p20"><Button data-testid={"list_resource__create_button_" + createPathFirstPart} variant="contained" onClick={onAction}><AddIcon />{createLabel}</Button></div>}
