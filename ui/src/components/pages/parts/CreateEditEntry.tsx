...
function CreateEditEntry({ schema, path, data, readonly, org, t }: TempAny) {
  ...
  const [restCache, setRestCache] = useState<Map<string, any>>(
    new Map<string, any>(),
  );
  ...
  async function updateOrgProjDbEnums(
    data: TempAny,
    ...
    for (let i = 0; i < ret.length; i++) {
      if (ret[i].params["organization"]) {
        ...
        ret[i].params["organization"].enum = uniqueArray([
          ...getItemsFirstPart(users),
          ...getItemsFirstPart(projects),
        ]);
      }
      if (ret[i].params["project"]) {
        let enums: string[] = [];
        if (data.organization) {
          const projects = await cachedGet(
            "/projects/" +
              encodeURIComponent(data.organization) +
              "?listAccessible=true",
          );
      ...
      if (ret[i].params["database"]) {
        ...
        if (data.organization && data.project) {
          const databases = await cachedGet(
            "/databases/" +
              encodeURIComponent(data.organization) +
              "/" +
  ...
  useEffect(() => {
    ...
    function getFieldParameters(
      formParams: FieldParametersType,
      key: string,
    ): FieldParameterType | undefined {
      ...
      if (posPeriod === -1) {
        ...
        return (
          childForm &&
          childForm.properties &&
          getFieldParameters(childForm.properties, key.substring(posPeriod + 1))
        );
      }
    }
    ...
    function deleteFieldParameters(
      formParams: FieldParametersType,
      key: string,
    ) {
      const posPeriod = key.indexOf(".");
    ...
    if (resource["put"]) {
      formParams = getChild(resource["put"], [
        "requestBody",
        "content",
        "application/json",
        "schema",
      ...
      required = getChild(resource["put"], [
        "requestBody",
        "content",
        "application/json",
        "schema",
      ...
      formParams = getChild(resource["get"], [
        "responses",
        "200",
        "content",
        "application/json",
      ...
      formParams = cloneRecursive(formParams);
      required = getChild(resource["get"], [
        "responses",
        "200",
        "content",
        "application/json",
            ...
            if (key === "*") {
              ...
              if (fieldParameters) {
                Object.keys(section.fields[key]).forEach((fieldKey) => {
                  fieldParameters[fieldKey] = cloneRecursive(
                    section.fields[key][fieldKey],
                  );
                });
    ...
    const queryParameters =
      (urlParams &&
        Object.keys(urlParams)
          .filter(
            (key) =>
              urlParams[key].in === "query" &&
              (urlParams[key].name !== "allowCrossOrganizationAccess" ||
    ...
    updateOrgProjDbEnums(v, sectionFormParams).then(
      (sectionFormParams: SectionFormParameterType[]) => {
  ...
  useEffect(() => {
    ...
    updateOrgProjDbEnums(values, sectionFormParameters).then(
      (sectionFormParams: SectionFormParameterType[]) => {
        setSectionFormParameters(sectionFormParams);
      },
    );
      ...
      for (let r = 0; r < allowRules.length; r++) {
        ...
        if (ruleParts.length >= 2) {
          if (ruleParts[1].startsWith("/")) {
            ...
            ruleParts[1] !== values.organization &&
            !ruleParts[1].startsWith(values.organization + "/")
          ) {
            return true;
          }
        }
  ...
  async function handleSubmit() {
    ...
    if (hasOutOfOrgUserRules()) {
      ...
      if (data) {
        ...
        if (
          values.accessRule &&
          persistedValues.accessRule &&
          JSON.stringify(values.accessRule) ===
            JSON.stringify(persistedValues.accessRule)
        ) {
      ...
      if (
        ...
        "yes" ===
          (await Dialog.confirm(
            t("confirm.userAccessRuleCheck.title"),
            t("confirm.userAccessRuleCheck.body"),
            t,
          ))
      ) {
    ...
    submitForm(
      urlParameters,
      formParameters,
      data ? path : getCreatePath(schema, path),
      submitValues,
    )
      .then(() => {
        if (data) {
  ...
  function showSectionFields(section: TempAny) {
    let ret =
      (section &&
        section.params &&
        Object.keys(section.params).filter((key) => {
    ...
    ret = ret.map((key: string) => {
      ...
      const ro =
        readonly ||
        (data &&
          (key in urlParameters ||
            key === "name" ||
            formParameter["x-immutable"] === true)) ||
        (key === "organization" &&
          getSchemaPath(schema, path)?.includes("{organization}"));
      ...
      return (
        <div className="NuoFieldContainer" key={key}>
          {Field({
            op: "edit",
            path,
            prefix: key,
  ...
  if (values.status && readonly) {
    sectionsWithStatus.push(
      <Tab key="section-status" id="section-status" label="Status">
        {Field({
          op: "edit",
          path,
          prefix: "status",
  ...
  return (
    <>
      <ResourceHeader
        ...
        path={path}
        data={data}
        type={readonly ? "view" : data ? "edit" : "create"}
        onAction={() => {
          if (readonly) {
      ...
      <form>
        <div className="fields">
          ...
          {"_error" in errors && (
            <h3 style={{ color: "red" }}>{errors["_error"]}</h3>
          )}
          {"_errorDetail" in errors && (
            <div style={{ color: "red" }}>{errors["_errorDetail"]}</div>
          )}
        </div>
      </form>
    </>
