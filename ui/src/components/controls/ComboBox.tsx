...
function ComboBoxItems({
  ...
  return items.map((item, index) => (
    <div
      style={{ zIndex: 102, minWidth: position.width + "px" }}
      id={item.id}
      data-testid={item["data-testid"]}
      ref={ref[index]}
      key={item.id}
      className={
        "NuoMenuPopupItem" + (item.id === selected ? " NuoMenuSelected" : "")
      }
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      ...
      onKeyDown={(event) => {
        if (event.key === "Escape") {
          ...
          if (
            ...
            (index + 1 === items.length && !event.shiftKey)
          ) {
            event.preventDefault();
            event.stopPropagation();
          }
    ...
    return (
      <div>
        <div
          ...
          className={
            "NuoMenuPopup " +
            (align === "right" ? " NuoAlignRight" : " NuoAlignLeft")
          }
          style={{
            maxHeight:
              String(window.innerHeight - position.y - position.height - 5) +
              "px",
            overflowY: "auto",
            padding: "0",
            margin: "0",
