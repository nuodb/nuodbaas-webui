...
export default function Menu(props: MenuProps): JSX.Element {
  ...
  const [popupAnchor, setPopupAnchor] = useState<Element | undefined>(
    undefined,
  );
  ...
  const mainItem = items.find(
    (item) => !!defaultItem && item.id === defaultItem,
  );
  ...
  if (children) {
    return (
      <div className={mainItem && "NuoButtonDown"}>
        {mainItem && (
          <button
            data-testid={mainItem["data-testid"]}
            onClick={mainItem.onClick}
          >
            <div className="NuoIcon">{mainItem.icon}</div>
            {mainItem.label}
          </button>
        )}
        <div
          className="NuoRowFixed"
          tabIndex={0}
        ...
        >
          <div className="NuoColumn">
            ...
            <div className="NuoRow">
              {mainItem && (
                <div
                  className="NuoColumn NuoRight"
                  style={{ border: "1px solid #FFFFFF80" }}
                ></div>
              )}
              <div className="NuoColumn NuoRight">{children}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return items.map((item: MenuItemProps, index: number) => (
    <div
      ...
      onKeyDown={(e) => {
        if (e.key === "Enter") {
          ...
          e.key === "Tab" &&
          ((index === 0 && e.shiftKey) ||
            (index + 1 === items.length && !e.shiftKey))
        ) {
          e.preventDefault();
          e.stopPropagation();
    ...
    >
      <div className="NuoRowFixed">
        <div className="NuoIcon">{item.icon}</div>
        {item.label}
      </div>
      {(draggable === true && (index !== 0 || items[0].id !== "name") && (
        <DragHandleIcon />
      )) || <div></div>}
    </div>
  ));
...
export function PopupMenu(props: PopupMenuProps) {
  ...
  function dndOver(e: React.DragEvent<HTMLDivElement>) {
    ...
    if (dndIsBefore(dndSelected, draggableTarget)) {
      ...
      draggableTarget.parentNode.insertBefore(
        dndSelected,
        draggableTarget.nextSibling,
      );
    }
  }
  ...
  return (
    <div style={{ position: "absolute" }}>
      <div
        ...
        className={
          "NuoMenuPopup " +
          (props.align === "right" ? " NuoAlignRight" : " NuoAlignLeft")
        }
        onKeyDown={(event) => {
          if (event.key === "Escape") {
        ...
        style={{
          maxHeight:
            String(window.innerHeight - anchorRect.y - anchorRect.height - 5) +
            "px",
