...
function isSameDomain(domain: string) {
    ...
    if(!remoteDomain || currentDomain.toLowerCase() !== remoteDomain.toLowerCase()) {
        return false;
    }
    else {
        return true;
...
export function RemoteStorageBoundary({children}:{children:React.JSX.Element[]|React.JSX.Element}) : React.JSX.Element {
    ...
    function handleEvents(event: MessageEvent) {
        ...
        if (!isSameDomain(new URL(event.origin).host)) {
            console.log("Domains don't match", new URL(event.origin).host, window.location.host);
            return;
        }

        if (event.data.action === "remoteStorageGetResponse" || event.data.action === "remoteStorageSetResponse") {
    ...
    const iframeMemo = useMemo(() => {
        ...
        const multiInstanceRegistryUrl = Date.now() > 0 ? "___NUODB_MULTI_INSTANCE_REGISTRY_URL___" : "never occurs";
        if (!multiInstanceRegistryUrl.endsWith(".json") && multiInstanceRegistryUrl) {
...
const transactions = new Map<number, (value: TransactionResponse | PromiseLike<TransactionResponse>) => void>();
...
export class remoteStorage {
    ...
    public static async fillCache(key: string): Promise<TransactionResponse> {
        const iframe = document.getElementById('remoteStorage') as HTMLIFrameElement | null;
        if(!iframe) {
        ...
        const response = await new Promise<TransactionResponse>((resolve) => {
            ...
            iframe.contentWindow?.postMessage({ transactionId, action: 'remoteStorageGet', key }, window.location.origin);
        });
        if(response === null) {
    ...
    public static set = async (key: string, value: string | null) : Promise<TransactionResponse> => {
        const iframe = document.getElementById('remoteStorage') as HTMLIFrameElement | null;
        if(!iframe) {
            return Promise.resolve(null);
        }
        ...
        return new Promise<TransactionResponse>((resolve) => {
            transactions.set(transactionId, resolve);
            iframe.contentWindow?.postMessage({ transactionId, action: 'remoteStorageSet', key, value }, window.location.origin);
        });
    }
}
