...
export function Tabs({
  ...
  children = children.filter(
    (child: any) => child.props.id && child.props.label,
  );

  return (
    <div className="NuoTabs">
      <ul>
        {children.map((child: any, index) => (
          <li
            ...
            tabIndex={
              index === currentTab && children.length > 1 ? 0 : undefined
            }
          ...
          >
            {child.props.label}
            {badges && index in badges && badge(badges[index])}
          </li>
        ))}
        <li style={{ display: "flex", flex: "1 1 auto" }}></li>
      </ul>
      <div className="NuoTabsBody">{children[currentTab]}</div>
    </div>
  );
}
