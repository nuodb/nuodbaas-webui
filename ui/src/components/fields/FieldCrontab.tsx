...
export default function FieldCrontab(props: FieldProps): ReactNode {
  ...
  function edit(): ReactNode {
    ...
    return (
      <div key={prefix} className={(isOther && "NuoFieldCrontab") || ""}>
        ...
        >
          ...
          <SelectOption value="@hourly">
            {t("field.enum.frequency.@hourly")}
          </SelectOption>
          <SelectOption value="@daily">
            {t("field.enum.frequency.@daily")}
          </SelectOption>
          <SelectOption value="@weekly">
            {t("field.enum.frequency.@weekly")}
          </SelectOption>
          <SelectOption value="@monthly">
            {t("field.enum.frequency.@monthly")}
          </SelectOption>
          <SelectOption value="@yearly">
            {t("field.enum.frequency.@yearly")}
          </SelectOption>
          <SelectOption value="other">
            {t("field.enum.frequency.other")}
          </SelectOption>
        </Select>
        {isOther && (
          <div className="NuoCrontabContainer">
            {["minute", "hour", "dayOfMonth", "month", "weekday"].map(
              (fieldname, index) => (
                <TextField
  ...
  function validate(): boolean {
    ...
    if (
      ["@hourly", "@daily", "@weekly", "@monthly", "@yearly"].includes(value)
    ) {
      updateErrors(prefix, null);
      return true;
    }
