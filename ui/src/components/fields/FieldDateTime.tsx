      ...
      <div className="NuoCalendarDate">
          ...
          <div className="NuoCalendarMonthYear">
            ...
            >
              {months.map((month, monthIndex) => {
                return (
                  <option key={String(monthIndex)} value={String(monthIndex)}>
        ...
        <div className="NuoCalendarWeekdays">
          {weekdays.map((weekday) => (
            <div key={weekday} className="NuoCalendarCellHeader">
              {props.t("calendar.dayOfWeek." + weekday).charAt(0)}
            </div>
          ))}
        </div>
        {[0, 1, 2, 3, 4, 5].map((row) => {
          return (
            <div key={row} className="NuoRow">
              {weekdays.map((weekday, weekdayIndex) => {
                const index = start + row * 7 + weekdayIndex;
                if (index < 1 || index > end) {
                  return (
                    <div key={weekdayIndex} className="NuoCalendarCell"></div>
                  );
                } else {
...
export default function FieldDateTime(props: FieldProps): ReactNode {
    ...
    return (
      <TextField
        ...
        iconOnClick={async () => {
          ...
          if (
            "ok" ===
            (await Dialog.okCancel(
              t("field.label." + prefix),
              <CalendarPopup
                value={value}
                t={props.t}
            ...
            setValue(
              v,
              prefix,
              selectedValue.toISOString().replaceAll(".000Z", "Z"),
            );
            setValue(v, "_" + prefix);
        ...
        onBlur={() => {
          ...
          if (editValue === "") {
            ...
            if (isNaN(date.getTime())) {
              updateErrors(
                prefix,
                'Field "' + prefix + '" has invalid date/time value',
              );
              return;
            }
  ...
  function validate(): boolean {
    ...
    if (editValue !== null) {
      updateErrors(
        prefix,
        'Field "' + prefix + '" has invalid date/time format',
      );
      return false;
    }
