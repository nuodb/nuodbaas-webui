...
export default function FieldPassword(props: FieldProps): ReactNode {
  ...
  switch (props.op) {
    case "edit":
      return edit();
    case "view":
      return FieldBase_display(props);
    case "validate":
  ...
  function edit(): ReactNode {
    if (
      matchesPath(path, "/databases/{organization}/{project}/{database}") &&
      !window.location.pathname.startsWith("/ui/resource/create/")
    ) {
      return null;
    }
    ...
    return (
      <TextField
        ...
        error={error}
        onBlur={() => FieldBase_validate(props)}
        disabled={readonly}
      />
    );
  }
