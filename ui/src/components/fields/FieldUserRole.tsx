...
export default function FieldUserRole(props: FieldProps): ReactNode {
  const [parametersByTemplate, setParametersByTemplate] = useState<
  ...
  function edit(): ReactNode {
    ...
    ret.push(
      <div key="name" className="NuoFieldContainer">
        {Field({
          ...props,
          setValues: (values) => {
            ...
            if (props.values.roles.length <= v.roles.length) {
              for (let i = 0; i < v.roles.length; i++) {
                if (
                  props.values.roles.length < v.roles.length ||
                  props.values.roles[i].name !== v.roles[i].name
                ) {
                  v.roles[i].params = {};
                  const variables = parametersByTemplate
                    ? parametersByTemplate[v.roles[i].name] || []
                    : [];
    ...
    if (defaultName) {
      ...
      if (defaultParams && Object.keys(defaultParams).length > 0) {
        ...
        ret.push(
          <div key="params" className="NuoFieldContainer">
            <FieldMap
              {...props}
              values={allValues}
              prefix={prefixParams}
