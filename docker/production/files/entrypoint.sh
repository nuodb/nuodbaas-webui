...
if [ "${NUODB_MULTI_INSTANCE_JSON}" != "" ] ; then
    sed -i "s#___NUODB_MULTI_INSTANCE_REGISTRY_URL___#/ui/multiinstance.json#g" /usr/share/nginx/html/ui/config.json
...
find /usr/share/nginx/html -type f | while read line; do
    ...
    sed -i "s#___NUODB_MULTI_INSTANCE_REGISTRY_URL___#${NUODB_MULTI_INSTANCE_REGISTRY_URL}#g" ${line}
